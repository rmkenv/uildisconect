...
if merged.empty:
    disc_abbrs    = disc['state_abbr'].tolist()
    centers_abbrs = centers['state_abbr'].tolist()
    raise SystemExit(
        'Joined dataframe is empty.\n'
        'disconnections abbrs : ' + str(disc_abbrs) + '\n'
        'datacenters abbrs    : ' + str(centers_abbrs)
    )
...
for _, row in merged.iterrows():
    ...
    state_name = str(row['state_disc'])
    disc_count = int(row['disconnections_2024'])
    customers  = int(row['customers_2024'])
    rate       = float(row['disc_per_100'])
    dc_count   = int(row['dc_count'])
    dc_mw      = int(row['dc_mw'])

    popup_html = (
        '<b>' + state_name + '</b><br>'
        'Disconnections: ' + f'{disc_count:,}' + '<br>'
        'Customers: '      + f'{customers:,}'  + '<br>'
        'Rate: '           + f'{rate:.2f}'     + ' per 100<br>'
        'Data centers: '   + str(dc_count)     + ' (~' + str(dc_mw) + ' MW)'
    )
